...
### Question A
1. Meaning of <NULL>: There aren't any of this species on these islands, so there are no values so its an empty cell. 
...
```{r}
#create the desired table (I dropped the grouping, this was unclear in the directions)
penguins2 <- penguins |>
    group_by(species) |>
  summarise(
    Bisco = sum(island == "Biscoe", na.rm = TRUE),
    Dream = sum(island == "Dream", na.rm = TRUE),
    Torgersen = sum(island == "Torgersen", na.rm = TRUE),
    .groups = "keep"
